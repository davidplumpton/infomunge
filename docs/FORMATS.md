...
| --- | --- | --- | --- | --- | --- | --- |
| JSON | `application/json` | — | `.json` | Yes | Yes | Structured. Integer tokens within the runtime signed-integer range remain exact; out-of-range integers are rejected. Decimal and exponent tokens use finite IEEE 754 binary64 values. Output is compact JSON. |
...
| CSV | `application/csv` | `text/csv` | `.csv` | Yes | Yes | Structured. The first input row is the header and all input fields are strings. Output requires an array of objects; headers follow each key's first appearance while scanning rows and preserve insertion order within ordered objects (legacy unordered maps use lexical key order). Missing fields are empty, and nested values render as compact JSON text. |
| NDJSON | `application/x-ndjson` | `application/ndjson` | `.ndjson`, `.jsonl` | Yes | Yes | Structured. Each line uses the JSON numeric policy: in-range integers remain exact, out-of-range integers are rejected, and decimals/exponents use finite binary64 values. Blank input lines are skipped; output requires an array and emits one compact JSON value per line. |
