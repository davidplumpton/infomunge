...
- Numeric strings are coerced for unary negation and for arithmetic with a numeric operand (`+`, `-`, `*`, `/`, `mod`, and `%`). Other string operands to `+` use InfoMunge's string-concatenation extension; DataWeave-compatible scripts should use `++` for string concatenation.
- With `<`, `<=`, `>`, and `>=`, a numeric string and a number are compared numerically in either operand order; integer-shaped strings retain exact arbitrary-precision comparison. Nonnumeric strings remain incompatible with numbers. Collection ordering does not apply this coercion and rejects mixed Number/String keys.
- With `-`, an array on the left removes every element structurally equal to the right operand. An absent value leaves the array unchanged; numeric subtraction and object key removal retain their existing behavior.
- Decimal integer literals outside the native signed-integer range use an exact arbitrary-precision representation. Core arithmetic involving one of these values remains exact when it produces an integer; arithmetic involving only native-range integers retains checked-overflow errors. Decimal and exponent literals continue to use finite IEEE 754 binary64 values.
- InfoMunge supports `%` as a modulo operator. DataWeave-compatible scripts should use the left-associative `mod` operator instead. Like DataWeave, infix `mod` binds more tightly than `default`, `onNull`, `then`, ranges, and keyword operators such as `joinBy` and `contains`, but less tightly than logical operators, equality and ordering comparisons, type operators, `+`, `-`, `*`, `/`, and `%`. For example, `10 mod 3 == 1` is read as `10 mod (3 == 1)` and fails because the divisor is Boolean; write `(10 mod 3) == 1` when comparing the remainder.
...
- Ordinal selectors on numbers index the number's standard string coercion, including a leading minus sign, decimal point, or scientific-notation exponent. Negative indexes count from the end, and indexes outside either bound return `null`.
...
- Missing fields on external input objects retain an internal absence state through unary, binary, function, and callback expressions so a surrounding `default` can supply its fallback. Missing fields on objects created inside the script remain ordinary `null`; operations that reject explicit `null` still fail instead of being swallowed by `default`.
- Update expressions support a trailing `!` on selectors to upsert missing object fields. Missing upsert selectors bind `null`, and nested object selectors create their missing parent objects; selectors without `!` retain no-op behavior when the path is absent.
...
When the external DataWeave CLI is installed, the bounded suite generates 50
differential cases and requires at least 20% to reach structural comparison
after evaluator errors are classified. Run the larger 500-case budget explicitly:
