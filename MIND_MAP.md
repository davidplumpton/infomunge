...
[10] **Quality Gates** - Prefer targeted tests for the changed layer. Use `INFOMUNGE_SKIP_GODOG=1 go test ./... -timeout 5m` for the bounded repo-wide package suite, then run cucumber separately when user-visible behavior changed. In restricted workspaces where the default Go cache is unreadable, set `GOCACHE` to the repository-local `tmp/go-build-cache`. For documentation- or workflow-only edits, validate the affected commands, links, examples, or formatting; cucumber is unnecessary unless runtime behavior changes. The mutation corpus soak is skipped by default and runs explicitly with `INTENSIVE_TEST_SOAK=1`.
