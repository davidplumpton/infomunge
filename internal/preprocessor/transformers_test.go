...
func TestReplaceArrowFunctions(t *testing.T) {
	tests := []struct {
		...
		{"identifier pipeline in collection callback", `[0] map (ignored) -> xs map (x) -> x`, `[0] map __lambda("ignored", xs map (x) -> x)`},
		{"identifier pipeline in function argument", `apply(xs, (items) -> items filter (x) -> x)`, `apply(xs, __lambda("items", items filter (x) -> x))`},
		{"identifier pipeline in then callback", `value then (items) -> items reduce (x, acc) -> acc + x`, `value then __lambda("items", items reduce (x, acc) -> acc + x)`},
		{"identifier pipeline in onNull callback", `value onNull () -> items mapObject (v, k) -> v`, `value onNull __lambda("", items mapObject (v, k) -> v)`},
		{"top level identifier lambda keeps outer collection chain", `(x) -> x map (y) -> y`, `__lambda("x", x) map __lambda("y", y)`},
		{"scalar callback body keeps outer collection chain", `[1] map (x) -> x + 1 filter (y) -> y > 1`, `[1] map __lambda("x", x + 1) filter __lambda("y", y > 1)`},
...
func TestReplaceCollectionOperator(t *testing.T) {
	tests := []struct {
		...
		{"filter binds before equality comparison", `expected == values filter __lambda("x", x == x)`, `expected == __filter(values, __lambda("x", x == x))`, replaceFilterOperator},
		{"filter binds before inequality comparison", `null != values filter __lambda("x, i", i == i)`, `null != __filter(values, __lambda("x, i", i == i))`, replaceFilterOperator},
		{"filter binds before ordering comparison", `lower < values filter __lambda("x", x >= 0)`, `lower < __filter(values, __lambda("x", x >= 0))`, replaceFilterOperator},
		{"filter binds before inclusive ordering comparison", `lower <= values filter __lambda("x", x > 0)`, `lower <= __filter(values, __lambda("x", x > 0))`, replaceFilterOperator},
...
func TestReplaceAsOperator(t *testing.T) {
	tests := []struct {
		...
		{"as preserves grouped mod operand", "(5 mod 2) as String", `__coerce((5 mod 2), "String")`},
		{"as preserves grouped contains operand", `("abc" contains "a") as String`, `__coerce(("abc" contains "a"), "String")`},
		{"as ignores boundary operators in strings", `"left mod right" as String`, `__coerce("left mod right", "String")`},
...
func TestReplaceIsOperator(t *testing.T) {
	tests := []struct {
		...
		{"is preserves grouped mod operand", "(5 mod 2) is Number", `__isType((5 mod 2), "Number")`},
		{"is preserves grouped contains operand", `("abc" contains "a") is Boolean`, `__isType(("abc" contains "a"), "Boolean")`},
		{"is ignores boundary operators in strings", `"left contains right" is String`, `__isType("left contains right", "String")`},
